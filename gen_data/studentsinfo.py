...
def gen_gpa():
    gpalist = []
    result  = np.random.normal(2.2 , 0.6 , 90)
...
def gen_data(lessonno):
    ...
    for lesson in [colname  for colname in col_names if colname != 'gpa']:
        res_df[lesson] = res_df[lesson].astype('int')
    res_df = res_df.round({'gpa': 2})
    return res_df

def gen_alldata(pathname):
    for i in range(1,6):
        lessonno = 2130000
        filename = '.csv'
        df = gen_data(lessonno + i*100)
        df.to_csv(str(pathname + str(i)  + filename), sep = ',',index = False)

if __name__ == '__main__':
    #np.set_printoptions(precision=3,suppress=True)
    #print(df.sort_values(by="gpa" , ascending=True)['studentno'][:8] )
    datapath = 'data\\lessondata\\lessondata' 
    gen_alldata(datapath)
